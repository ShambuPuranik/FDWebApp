...
class AccountServiceTest {
    ...
    // ---------- In-memory accounts (A and B) ----------
    @Test
    void testDebitSuccess() {
        String response = accountService.debit("A", 1000.0);
        assertTrue(response.contains("Debited 1000.0 from A"));
        assertEquals(4000.0, accountService.getBalance("A"));
    }

    @Test
    void testDebitInsufficientBalance() {
        RuntimeException ex = assertThrows(RuntimeException.class,
                () -> accountService.debit("B", 5000.0));
        assertEquals("Insufficient balance in B", ex.getMessage());
    }

    @Test
    void testCreditSuccess() {
        String response = accountService.credit("B", 1000.0);
        assertTrue(response.contains("Credited 1000.0 to B"));
        assertEquals(3000.0, accountService.getBalance("B"));
    }
